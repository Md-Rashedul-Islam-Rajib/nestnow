...
const UpdateRentalHouseListing = async () => {
  ...
  return (
    <main className="container mx-auto py-10">
      <h1 className="text-3xl font-bold mb-6 text-center">
        Available for update Rental Houses 
      </h1>
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
        {rentalHouses?.map((rentalHouse) => (
          <RentalHouseCard key={rentalHouse._id} action='update' rentalHouse={rentalHouse} />
        ))}
      </div>
    </main>
  );
...
export default UpdateRentalHouseListing;